    ...
    def _parse_element(self, inp):
        ...
        if data.startswith("("):
            if data.endswith(")"):
                ...
                raise DTDParsingError("Element definition error: unbalanced parentheses"
            ...
            if data.startswith('('):
                ...
                if len(test) > 1:
                    if test[1].startswith(','):
                        data = test[1][1:]
                        if data == '':
                            raise DTDParsingError("Unfinished element definition"
                                                  " in line " + str(self._line))
                    else:
                        data = test[1]
                else:
            ...
            while data:
                ...
                if data.startswith("("):
                    ...
                    if data.startswith('"'):
                        ...
                        print("decl:", decl)
                        print("data:", data)
                    new = Attribute(self._dtd_dic[parent], name=name, type="ENUM", decl=decl,
                                    items=enum, value=item)
                    # if len(data) > 1:
                    #     data = data[1]
                    #     print "data:", data
                else:
                    ...
                    if decl == '#FIXED':
                        ...
                        print("data:", data)
                        new = Attribute(self._dtd_dic[parent], name=name, type=type, decl='fix',
                                        value=item)
                    elif decl.startswith('"'):
                        if decl.endswith('"'):
                            new = Attribute(self._dtd_dic[parent], name=name, type=type,
                                            decl='dflt', value=decl[1:-1])
                        else:
                            raise DTDParsingError('Incorrect attribute definition:'
                        ...
                        new = Attribute(self._dtd_dic[parent], name=name, type=type, decl=decl)
        except ValueError:
            raise DTDParsingError('Incorrect attribute definition')
