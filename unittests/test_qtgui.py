...
from axe import qtgui as testee
...
class TestGui:
    ...
    def test_edit_item(self, monkeypatch, capsys):
        ...
        #         "called UndoRedoStack.push\n"
        #         "called Editor.mark_dirty with arg True\n")
        testobj.edit_item('element', 'oldstate', 'newstate', 'command_text')
        assert capsys.readouterr().out == (
                "called EditCommand.__init__ with args"
                f" ({testobj}, 'oldstate', 'newstate', 'command_text') {{}}\n"
                "called UndoRedoStack.push\n")
    ...
    def test_paste(self, monkeypatch, capsys):
        ...
        assert testobj.item == item
        assert capsys.readouterr().out == (
                f"called PasteElementCommand.__init__ with args ({testobj}, 'Tag', 'Text', {item})"
                " {'before': True, 'below': False, 'description': 'Paste Element',"
                " 'data': (('', ('Tag', 'Text')),)}\n"
                "called UndoRedoStack.push\n"
                "called Editor.mark_dirty with arg True\n")
        ...
        assert capsys.readouterr().out == (
                f"called PasteElementCommand.__init__ with args ({testobj}, 'Tag', 'Text', {item})"
                f" {{'before': False, 'below': True, 'description': 'Paste Element',"
                " 'data': (('', ('Tag', 'Text')),)}\n"
                "called UndoRedoStack.push\n"
                "called Editor.mark_dirty with arg True\n")
    ...
    def test_add_attribute(self, monkeypatch, capsys):
        ...
        #         "called UndoRedoStack.push\n"
        #         "called Editor.mark_dirty with arg True\n")
        testobj.add_attribute('item', 'name', 'value', 'command')
        assert capsys.readouterr().out == (
                "called PasteAttributeCommand.__init__ with args"
                f" ({testobj}, 'name', 'value', 'item', 'command') {{}}\n"
                "called UndoRedoStack.push\n")
    ...
    def test_insert(self, monkeypatch, capsys):
        ...
        #         "called UndoRedoStack.push\n"
        #         "called Editor.mark_dirty with arg True\n")
        testobj.insert('item', 'tag', 'text', 'command')
        assert capsys.readouterr().out == (
                "called PasteElementCommand.__init__ with args"
                f" ({testobj}, 'tag', 'text', 'item')"
                " {'before': True, 'below': False, 'description': 'command'}\n"
                "called UndoRedoStack.push\n")
        testobj.insert('item', 'tag', 'text', 'command', before=False, below=True)
        assert capsys.readouterr().out == (
                "called PasteElementCommand.__init__ with args"
                f" ({testobj}, 'tag', 'text', 'item')"
    ...
    def test_init_gui(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == (
                ...
                f"called UndoredoStack.__init__ with arg {testobj}\n"
                "called Editor.mark_dirty with arg False\n")
        # expected_output['maingui2'].format(testobj=testobj)
    ...
    def test_on_keyup(self, monkeypatch, capsys):
        ...
        event = mockqtw.MockEvent(key='xxx')
        assert not testobj.on_keyup(event)
        assert capsys.readouterr().out == "called Tree.currentItem\n"
...
class TestVisualTree:
    ...
    def test_mouseDoubleClickEvent(self, monkeypatch, capsys):
        ...
        def mock_itemat_2(*args):
            ...
            return 'an item'
        def mock_edit(arg):
            print(f"called Editor.edit_item with arg '{arg}'")
        testobj = self.setup_testobj(monkeypatch, capsys)
        testobj.itemAt = mock_itemat
        testobj.parent.editor.edit = mock_edit
        event = mockqtw.MockEvent()
        ...
        testobj.mouseDoubleClickEvent(event)
        assert capsys.readouterr().out == ("called VisualTree.itemat with args ((1, 2),)\n"
                                           "called Editor.edit_item with arg 'an item'\n")
...
class TestPasteElementCommand:
    ...
    def test_init(self, monkeypatch, capsys):
        ...
        testobj = testee.PasteElementCommand(win, 'tag', 'text', 'yy', True, False, data=['xx'])
        ...
        testobj = testee.PasteElementCommand(win, 'tag', 'text', 'yy', False, False, "xx")
        assert testobj.win == win
        assert testobj.tag == 'tag'
        assert testobj.data == 'text'
        ...
        testobj = testee.PasteElementCommand(win, 'tag', 'text', 'zz', False, True, "xx", ['yy'])
        assert testobj.win == win
        assert testobj.tag == 'tag'
        assert testobj.data == 'text'
        assert not testobj.before
        ...
        testobj = testee.PasteElementCommand(win, 'tag', 'text', 'zz', True, True, "xx", ['yy'])
        assert testobj.win == win
        assert testobj.tag == 'tag'
        assert testobj.data == 'text'
        assert testobj.before
    ...
    def test_redo(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == (
                ...
                "called Editor.add_item with args ('added item #2', 'b', 'b')"
                " {'before': False, 'below': True, 'attr': False}\n"
                # "called Editor.add_item with args ('added item #3', 'c', 'dd')"
                "called Editor.add_item with args ('added item #3', 'd', 'd')"
...
class TestDialogGui:
    """unittests for qtgui.DialogGui
    """
    def setup_testobj(self, monkeypatch, capsys):
        """stub for qtgui.DialogGui object
    ...
    def test_add_label(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == (
                ...
                "called Grid.addWidget with arg MockLabel at ('row', 'col', 1, 2)\n")
    ...
    def test_add_checkbox(self, monkeypatch, capsys):
        """unittest for DialogGui.add_checkboxText
        """
        ...
        assert capsys.readouterr().out == (
                ...
                "called Grid.addWidget with arg MockCheckBox at ('row', 'col')\n")
    ...
    def test_add_textinput(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == (
                ...
                "called Grid.addWidget with arg MockEditorWidget at ('row', 'col', 1, 2)\n")

    def test_add_buttons(self, monkeypatch, capsys):
        """unittest for DialogGui.add_buttonso
        """
        monkeypatch.setattr(testee.qtw, 'QHBoxLayout', mockqtw.MockHBoxLayout)
        ...
        testobj.add_buttons([('xx', 'callback1', True), ('yy', 'callback2', False)])
        assert capsys.readouterr().out == (
                "called HBox.__init__\n"
                "called HBox.addStretch\n"
    ...
    def test_set_textinput_text(self, monkeypatch, capsys):
        ...
        """
        tb = mockqtw.MockEditorWidget()
        assert capsys.readouterr().out == "called Editor.__init__\n"
        testobj = self.setup_testobj(monkeypatch, capsys)
        testobj.set_textinput_text(tb, 'text')
        assert capsys.readouterr().out == "called Editor.setText with arg `text`\n"
    ...
    def test_get_lineinput_text(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == "called LineEdit.__init__ with args ()\n"
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.get_lineinput_text(tb) == ""
    ...
    def test_get_combobox_index(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == "called ComboBox.__init__\n"
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.get_combobox_index(cmb) == 1
    ...
    def test_get_combobox_itemtext(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == "called ComboBox.__init__\n"
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.get_combobox_itemtext(cmb, 'indx') == "indx"
        assert capsys.readouterr().out == "called ComboBox.itemText with value `indx`\n"
    ...
    def test_get_textinput_text(self, monkeypatch, capsys):
        ...
        assert capsys.readouterr().out == "called Editor.__init__\n"
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.get_textinput_text(tb) == "editor text"
    ...
    def test_keyPressEvent(self, monkeypatch, capsys):
        """unittest for DialogGui.keyPressEvent
        """
        def mock_key():
    ...
    def test_refresh(self, monkeypatch, capsys):
        ...
        def mock_update(self):
            print('called Dialog.update')
        monkeypatch.setattr(testee.qtw.QDialog, 'update', mock_update)
        testobj = self.setup_testobj(monkeypatch, capsys)
        testobj.sizer = mockqtw.MockVBoxLayout()
        assert capsys.readouterr().out == "called VBox.__init__\n"
        testobj.refresh()
        assert capsys.readouterr().out == ("called VBox.update\n"
                                           "called Dialog.update\n")
